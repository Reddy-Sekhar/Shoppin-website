...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'approval_status', 'approval_status_label', 'approval_notes', 'is_active',
        ]
...
class AdminUserUpdateSerializer(serializers.ModelSerializer):
    """Serializer for admins updating any user record."""

    class Meta:
        model = User
        fields = ['first_name', 'last_name', 'phone', 'company', 'role', 'is_active', 'approval_status', 'approval_notes']
        extra_kwargs = {
            'first_name': {'required': False},
            'last_name': {'required': False},
            'phone': {'required': False},
...
class UserCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        requested_role = (validated_data.get('role') or 'BUYER').strip().upper()
        allowed_roles = {choice[0] for choice in User.ROLE_CHOICES}
        role = requested_role if requested_role in allowed_roles else 'BUYER'

        approval_status = 'APPROVED'
        is_active = True
        if role in ('SELLER', 'DESIGNER'):
...
class CustomTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def validate(self, attrs):
        ...
        approval_status = getattr(self.user, 'approval_status', 'APPROVED')
        role = getattr(self.user, 'role', '').upper()
        if approval_status == 'PENDING':
            raise AuthenticationFailed('Your account is awaiting admin approval. You will receive an email once reviewed.')
        if approval_status == 'REJECTED':
            detail = self.user.approval_notes or 'Your account application was rejected. Please contact support for details.'
