...
const Products = () => {
    ...
    const filteredProducts = filter === 'all'
        ? products
        : products.filter(p => p.category.toLowerCase().includes(filter.toLowerCase()) || p.sub_category.toLowerCase().includes(filter.toLowerCase()));

    // Helper to get price range
    const getPriceRange = (product) => {
        if (!product.price_tiers || product.price_tiers.length === 0) return 'Inquire for Price';
        const prices = product.price_tiers.map(t => t.price);
        const minPrice = Math.min(...prices);
        const maxPrice = Math.max(...prices);
        return `$${minPrice.toFixed(2)} - $${maxPrice.toFixed(2)} / Piece`;
    };
                ...
                {filteredProducts.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                        {filteredProducts.map((product) => (
                            <div key={product.id} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-card transition-all duration-300 border border-slate-100">
                                ...
                                <div className="p-6">
                                    <div className="text-xs text-accent font-semibold uppercase tracking-wider mb-1">{product.sub_category}</div>
